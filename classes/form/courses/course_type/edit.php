...
class edit extends moodleform {
    ...
    protected function definition(): void {
        ...
        foreach ($fields as $field) {
            ...
            $mform->addElement('advcheckbox', $name, $leftlabel, $rightlabel = get_string('enable'), $groups = null, $values);
    ...
    public function validation($data, $files): array {
        ...
        // Contrôle que le champ 'shortname' est uniqe.
        ...
        // Contrôle la validité du champ 'fullnametemplate'.
        $fields = [];
        $categoryfield = false;
        $handler = course_handler::create();
        foreach ($handler->get_fields() as $field) {
            $placeholder = sprintf('%%%02d', $field->get('id'));

            if ($field->get('shortname') === 'category') {
                $categoryfield = new stdClass();
                $categoryfield->name = $field->get('name');
                $categoryfield->code = $placeholder;
            }

            $fields[$placeholder] = $field;
        }

        // Contrôle que le champ 'category' est présent dans le nom du template.
        if ($categoryfield !== false && str_contains($data['fullnametemplate'], $categoryfield->code) === false) {
            $errors['fullnametemplate'] = get_string('field_X_must_be_selected_code_X', 'local_apsolu', $categoryfield);
        }

        // Vérifie que le modèle de cours utilise uniquement des champs activés.
        ...
        foreach ($matches[0] as $code) {
            ...
            $field = $fields[$code];
            $shortname = $field->get('shortname');
            if (isset($data['fields'][$shortname]['fieldid']) === true && empty($data['fields'][$shortname]['fieldid']) === true) {
                $fieldname = sprintf('fields[%s][fieldid]', $shortname);

...
class report extends \local_apsolu\local\statistics\report {
    ...
    public function __construct() {
        ...
        $this->WithEnrolments = 'WITH enrolments AS (
            ...
            Sexe.data as sexe,
            CASE WHEN UFR.data IS NULL THEN \'\' ELSE UFR.data END AS ufr,
            CASE WHEN LMD.data IS NULL THEN \'\' ELSE LMD.data END AS lmd, apsoluhighlevelathlete.data as shnu,
            ...
            CASE WHEN (apsoluusertype.data IS NOT NULL AND apsoluusertype.data <> \'\')
              ...
              ELSE CASE WHEN (U.email LIKE \'%@etudiant.univ-%\' OR U.email LIKE \'%@etu.univ-%\' OR
                              U.email LIKE \'%@eleves.%\' OR U.email LIKE \'%@etud.univ-%\' OR U.email LIKE \'%@etudiant.%\')
            	THEN \'Étudiant\'
          ...
          LEFT JOIN {user_info_data} Sexe ON Sexe.userid = U.id AND
                                             Sexe.fieldid = (select id from mdl_user_info_field where shortname = \'apsolusex\')
          LEFT JOIN {user_info_data} UFR ON UFR.userid = U.id AND
                                            UFR.fieldid = (select id from mdl_user_info_field where shortname = \'apsoluufr\')
          LEFT JOIN {user_info_data} LMD ON LMD.userid = U.id AND
                                            LMD.fieldid = (select id from mdl_user_info_field where shortname = \'apsolucycle\')
          LEFT JOIN {user_info_data} apsoluhighlevelathlete ON apsoluhighlevelathlete.userid = U.id AND
                 apsoluhighlevelathlete.fieldid = (select id from mdl_user_info_field where shortname = \'apsoluhighlevelathlete\')
          LEFT JOIN {user_info_data} apsoluusertype ON apsoluusertype.userid = U.id AND
    ...
    public function getReportDisplay($datatype = null) {
        ...
        switch ($datatype) {
            ...
            default: // 1 : Vue activités physiques par inscription.
                ...
                [ 'data' => "cohortnames", 'visible' => false, 'title' => get_string('cohort', 'cohort')],
                [ 'data' => null, 'visible' => false, 'title' => "Activité détaillée", "render" => "function ( data, type, row ) {
                    return data.activityname.replace(/\s/g,'&nbsp;') + '&nbsp;/&nbsp;' +
                    moment.weekdays()[(data.slotnumweekday==7) ? 0 : data.slotnumweekday] +'&nbsp;/&nbsp;' +
                    data.slotstart + '&nbsp;-&nbsp;' + data.slotend + '&nbsp;/&nbsp;' + data.skillsname.replace(/\s/g,'&nbsp;');}"],
                ];
                $orders = [2 => 'asc', 3 => 'asc'];
    ...
    public function getReportData($querybuilder, $criterias = null) {
        ...
        if ($condition->datatype == 3 || $condition->datatype == 4) {
            $with = $this->WithComplementary;
        }
        ...
        switch ($condition->datatype) {
            ...
            case 4:
                ...
                $select = 'SELECT DISTINCT e.userid,e.idnumber,e.firstname,e.lastname,e.email,e.sexe,e.institution,
                                           e.department, e.ufr, e.lmd, e.userprofile,
    ...
    public function get_nowishes_list($params) {
        ...
        $sql = $params["WithEnrolments"] . 'SELECT DISTINCT U.id,U.idnumber, U.firstname, U.lastname,
                                                            U.email,U.institution,U.department
        FROM {user} U
    ...
    public static function custom_shnu($params) {
        ...
        FROM (
          ...
          FROM (" . $params["WithEnrolments"] . "	SELECT DISTINCT e.userid,e.idnumber,e.firstname,e.lastname,e.email,e.sexe,
            e.institution, e.department, e.ufr, e.lmd, e.userprofile,
          	COUNT(CASE WHEN status=0 THEN 1 ELSE NULL END) AS accepted_list
          	FROM enrolments e
          	WHERE shnu = true
          	GROUP BY e.userid
          ...
          UNION
          SELECT DISTINCT userid,idnumber,firstname,lastname,email,sexe,institution, department, ufr, lmd, userprofile
          FROM (" . $params["WithComplementary"] . " SELECT DISTINCT e.userid,e.idnumber,e.firstname,e.lastname,e.email,e.sexe,
            e.institution, e.department, e.ufr, e.lmd, e.userprofile
			     FROM enrolments e
			     WHERE shnu = true
          ) SET2
    ...
    public static function custom_apsoluhighlevelathlete($params) {
        ...
        FROM mdl_user u
        LEFT JOIN mdl_user_info_data ufr ON ufr.userid = u.id AND
                                            ufr.fieldid = (select id from mdl_user_info_field where shortname = 'apsoluufr')
        LEFT JOIN mdl_user_info_data apsoluhighlevelathlete ON apsoluhighlevelathlete.userid = u.id AND
              apsoluhighlevelathlete.fieldid = (select id from mdl_user_info_field where shortname = 'apsoluhighlevelathlete')
    ...
    public function enrollment_accepted_evaluated($params) {
        ...
        SELECT r.id,r.shortname,
            CONCAT('{\"id\": \"roleshortname\", \"operator\": \"contains\", \"value\": \"',shortname,'\"}') AS rules
        FROM mdl_role r
        ...
        {\"id\": \"institution\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.institution = '' OR e.institution IS NULL)
            THEN ' ' ELSE e.institution END,'\"}]}') AS institution_rules,
        e.ufr, SUM(COUNT(e.enrolid)) OVER (PARTITION BY e.institution,e.ufr) AS ufr_total,
        CONCAT('{\"condition\":\"AND\",\"rules\":[{\"id\":\"status\",\"operator\":\"equal\",\"value\": \"0\"},
        {\"condition\": \"OR\",\"rules\": [',(SELECT GROUP_CONCAT(rules ORDER BY rules SEPARATOR ',') FROM gradable),']},
        {\"id\": \"institution\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.institution = '' OR e.institution IS NULL)
            THEN ' ' ELSE e.institution END,'\"},
        {\"id\": \"ufr\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.ufr = '' OR e.ufr IS NULL)
            THEN ' ' ELSE e.ufr END,'\"}]}') AS ufr_rules,
        ...
        {\"id\": \"ufr\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.ufr = '' OR e.ufr IS NULL)
            THEN ' ' ELSE e.ufr END,'\"},
        {\"id\": \"department\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.department = '' OR e.department IS NULL)
            THEN ' ' ELSE e.department END,'\"}]}') AS department_rules,
        ...
        {\"id\": \"ufr\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.ufr = '' OR e.ufr IS NULL)
            THEN ' ' ELSE e.ufr END,'\"},
        {\"id\": \"department\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.department = '' OR e.department IS NULL)
            THEN ' ' ELSE e.department END,'\"},
        {\"id\": \"lmd\", \"operator\": \"equal\", \"value\": \"',CASE WHEN (e.lmd = '' OR e.lmd IS NULL)
            THEN ' ' ELSE e.lmd END,'\"}]}') AS lmd_rules
      FROM enrolments e

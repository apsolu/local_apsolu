define(['jquery','core/templates','enrol_select/select2'],function($,templates){return{initialise:function(){function filter(){var filters={};$('#apsolu-enrol-select tbody tr').css('display','table-row');$('.apsolu-enrol-selects').each(function(){var values=$(this).select2('data');var target=$(this).attr('name');if(values.length===0){return;}
...
$(this).css('display','none');});if(headerChildren===0){header.css('display','none');}});if(Object.keys(filters).length!==0){var permalink=document.getElementById('apsolu-offerings-permalink-a');if(permalink){var anchor=window.btoa(unescape(encodeURIComponent(JSON.stringify(filters))));permalink.setAttribute('href','#'+anchor);}}}

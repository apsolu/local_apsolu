...
data=new Object();data.userid=$(this).data('userid');data.courseid=$(this).data('courseid');data.listid=$(this).data('listid');data.ueid=$(this).data('ueid');data.roleid=$(this).data('roleid');data.raid=$(this).data('raid');$.ajax({url:M.cfg.wwwroot+"/local/apsolu/attendance/ajax/edit_enrolment.php",type:'POST',data:data,dataType:'json'}).done(function(result){try{$('#apsolu-attendance-popup').html(result.form);apsolu_attendance_handle_edit_enrolments_form();}catch(e){console.log(e);}});popup.css({backgroundColor:'#EEEEEE',padding:'.5em',cursor:'default',maxWidth:'50%',textAlign:'justify'});popup.popup('show');});function apsolu_attendance_handle_edit_enrolments_form(){$('#apsolu-attendance-ajax-edit-enrolment form').submit(function(event){event.preventDefault();$.ajax({url:M.cfg.wwwroot+"/local/apsolu/attendance/ajax/edit_enrolment.php",type:'POST',data:$('#apsolu-attendance-ajax-edit-enrolment form').serialize(),dataType:'json'}).done(function(result){$('#apsolu-attendance-popup').html(result.form);var list=$('.apsolu-attendance-status[data-ueid='+result.ueid+']');if(list.length==1){list.html(result.list);$('.apsolu-attendance-edit-enrolments[data-ueid='+result.ueid+']').data('listid',result.listid);}
var role=$('.apsolu-attendance-role[data-raid='+result.raid+']');if(role.length==1){role.html(result.role);$('.apsolu-attendance-edit-enrolments[data-raid='+result.raid+']').data('roleid',result.roleid);}

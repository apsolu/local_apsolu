...
try {
    ...
    switch ($action) {
        ...
        case 'check_balance':
            ...
            $soldeeur = atouts_manager::get_solde_disponible($nocarte);

            echo json_encode([
                'success'      => true,
                'solde_raw'    => $soldeeur, // Valeur numérique pour calculs JS.
                'solde_format' => number_format($soldeeur, 2, ',', ' ') . ' €', // Valeur formatée pour l'affichage.
            ]);
            break;
        ...
        case 'apply_deduction':
            ...
            $atoutrecord = new stdClass();
            $atoutrecord->paymentid   = $paymentid;
            $atoutrecord->userid      = $USER->id;
            $atoutrecord->nocarte     = trim($nocarte); // Nettoyage des espaces pour la BDD.
            $atoutrecord->amount      = $amount;
            $atoutrecord->status      = 0; // 0 = En attente (sera passé à 1 par paybox.php lors du succès CB).
            $atoutrecord->ticket      = 0;
            $atoutrecord->timecreated = time();
            ...
            if ($existing) {
                ...
                $atoutrecord->id = $existing->id;
                $DB->update_record('apsolu_atouts_payments', $atoutrecord);
            } else {
                // Création du premier enregistrement.
                $DB->insert_record('apsolu_atouts_payments', $atoutrecord);
            }

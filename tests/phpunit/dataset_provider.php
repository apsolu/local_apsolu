...
namespace local_apsolu\tests\phpunit;
...
use loca_apsolu\core\course;

define('APSOLU_DEMO', true); // Permet de générer des jeux de données sur les sessions passées (sessions, présences...).

/**
 * Classe aidant à la génération d'un jeu de données pour les tests unitaires.
 *
...
class dataset_provider {
        ...
        foreach ($courses as $course) {
            ...
            foreach ($sessions as $session) {
                ...
                foreach ($enrolment->users as $user) {
                    ...
                    // Ajoute des QR codes aux sessions à venir pour les cours de l'enseignante.
                    $qrcode = new qrcode();
    ...
    private static function setup_courses() {
        ...
        // Ajoute du contenu spécifique pour l'instance de test.
        $testcontent = [];

        $testcontent[] = str_getcsv('Paris,Vaires-sur-Marne,Base nautique de Vaires-sur-Marne,Mairie de Paris,' .
        ...
        $testcontent[] = str_getcsv('Paris,Saint-Denis,Stade de France,Mairie de Paris,Athlétisme (Réservation à la séance),,' .
            'Sports athlétiques,Expert,Jeudi,08:30,10:00,AnnuelleSP,Service des sports,lenseignante', ',', '"', '');
        $testcontent[] = str_getcsv('Paris,Paris,Grand Palais éphémère,Mairie de Paris,Aïkido (Réservation à la séance),,' .
            'Sports de combat,Débutant,Vendredi,08:30,10:00,AnnuelleSP,Service des sports,lenseignante', ',', '"', '');

        $first = true;
        while (($data = fgetcsv($handle, 0, ',', '"', '')) !== false || $testcontent !== []) {
            if ($first === true) {
            ...
            if ($data === false) {
                // Utilise les valeurs présentes dans $testcontent.
                $data = array_shift($testcontent);
            }
            ...
            // Pas de filtre sur les villes : tous les cours sont créés (cf. test/behat/dataset_provider.php).
            ...
            if ($fullname === 'Basket-ball 3x3 (H) Jeudi 15:30 17:30 Expert') {
                $teachersarr = explode(',', $teachers);
                $teacheradmin = $DB->get_record('user', ['username' => 'teacheradmin', 'deleted' => 0], $fields = '*', MUST_EXIST);
                $teachersarr[] = $teacheradmin->username;
                $teachers = implode(',', $teachersarr);
            }

            self::setup_enrolments($course, $period, $teachers);

            // Ajoute du contenu dans l'espace-cours.
            if ($fullname === 'Basket-ball 5x5 (H) Jeudi 14:30 16:15 Débutant') {
                $sections = [];
        ...
        // TODO : Ajouter un cours meta + inscriptions.
    }
    ...
    private static function setup_federation_course() {
        ...
        foreach ($DB->get_records('user', ['deleted' => 0]) as $user) {
            ...
            if ($user->username == 'etudiant16') {
                $adhesionuserid = $user->id;
            }
            if ($user->username == 'etudiant17') {
                $enrolmentuserid = $user->id;
            }

            cohort_add_member($cohort->id, $user->id);
        }

        // Ajoute des inscriptions (méthode select) pour 2 étudiants (etudiant16 et etudiant17).
        $plugin->enrol_user(
            $enrol,
            $enrolmentuserid,
            $apsolurole->id,
        ...
        // Ajoute une adhésion.
        $adhesion = (object) ['birthday' => 1002718703, 'userid' => $adhesionuserid];
        $DB->insert_record('apsolu_federation_adhesions', $adhesion);
    }
    ...
    private static function setup_payments(array $courses, array $centers) {
        ...
        // Ajoute des informations de paiments.
        $addresses = ['4 rue des Lilas', '17 avenue Pasteur', '3 impasse du Moulin', '22 boulevard Carnot', '8 rue de la Paix',
            '51 chemin des Vignes', '2 place du Marché', '30 rue Voltaire', '11 allée des Cèdres', '6 route de Lyon'];
        $cities = ['Lyon', 'Nantes', 'Paris', 'Reims', 'Nice'];

        // On choisit un groupe d'utilisateurs (ceux qui sont inscrits dans IUT Paris) pour générer des id utilisateurs.
        $sql = "SELECT * FROM {user} u WHERE u.institution = :institution LIMIT 6";
        $usersgroup = $DB->get_records_sql($sql, ['institution' => 'IUT Paris']);

        foreach ($usersgroup as $user) {
            $rdmkey = array_rand($addresses);
            $address = $addresses[$rdmkey];
            unset($addresses[$rdmkey]);
            $city = $cities[array_rand($cities)];

            $payment = new stdClass();
            $payment = (object) ['id' => random_int(99, 999), 'timecreated' => time() - random_int(5000, 50000), 'method' => 'card',
                'source' => 'apsolu', 'userid' => $user->id, 'paymentcenterid' => 2, 'status' => 3,
                'amount' => random_int(10, 100)];

            $DB->insert_record('apsolu_payments', $payment);

            $paymentaddresses = (object) ['firstname' => $user->firstname, 'lastname' => $user->lastname,
                'address1' => $address, 'city' => $city, 'countrycode' => 33, 'userid' => $user->id,
                'timecreated' => $payment->timecreated];

            $DB->insert_record('apsolu_payments_addresses', $paymentaddresses);

            $paymentitems = (object) ['paymentid' => $payment->id, 'cardid' => 2];
            $DB->insert_record('apsolu_payments_items', $paymentitems);

            $paymenttransactions = (object) ['timecreated' => $payment->timecreated, 'reference',
                bin2hex(random_bytes(2)) . '-' . $payment->id, 'amount' => $payment->amount];
            $DB->insert_record('apsolu_payments_transactions', $paymenttransactions);
        }
    }
     ...
     * - 15 enseignants
     * - 60 étudiants
     * - 2 administrateurs supplémentaires (un compte Shibboleth et un administrateur / enseignant compte manuel)
     *
     * @return void
     */
    public static function setup_users() {
        global $DB;
        global $CFG;
        $generator = new testing_data_generator();
        ...
        // Génère des données pour les 3 utilisateurs de test.
        $password = null;

        $lastaccess = time() - (10 * 24 * 3600); // Dernière connexion il y a 10 jours.
        ...
        $users[] = ['username' => 'letudiant', 'password' => $password, 'idnumber' => '20160001', 'firstname' => 'Léo',
            ...
            'firstaccess' => $lastaccess, 'lastaccess' => $lastaccess];
        ...
        // User administrateurs Shibboleth.
        $users[] = ['username' => 'useradmin', 'password' => $password, 'firstname' => 'Admin', 'lastname' => 'User',
            'auth' => 'shibboleth', 'institution' => 'U. Rennes', 'sex' => 'F', 'policyagreed' => 1, 'type' => 'employee',
            'firstaccess' => $lastaccess, 'lastaccess' => $lastaccess];

        // User administrateur + enseignant (manuel).
        $users[] = ['username' => 'teacheradmin', 'password' => $password, 'firstname' => 'Admin Teacher', 'lastname' => 'User',
            'auth' => 'manual', 'institution' => 'U. Rennes', 'sex' => 'F', 'policyagreed' => 1, 'type' => 'employee',
            'firstaccess' => $lastaccess - (365 * 24 * 3600), 'lastaccess' => $lastaccess - (365 * 24 * 3600)];
        ...
        for ($i = 1; $i < 15; $i++) {
            ...
            $access = time() - ($i * 24 * 3600);
            if ($i == 14) {
                $access = $access - (365 * 24 * 3600); // Un enseignant ne s'est pas connecté depuis plus d'un an.
            }

            $users[] = ['username' => sprintf('enseignant%s', $i + 1), 'firstname' => $firstname, 'lastname' => $lastname,
                'sex' => $sex, 'institution' => 'U. Paris', 'policyagreed' => 1, 'type' => 'employee',
                'firstaccess' => $access, 'lastaccess' => $access];
        }
        ...
        for ($i = 1; $i < 60; $i++) {
            ...
            // Informations sur les connexions.
            $access = time() - ($i * 12 * 3600);
            if ($i > 57) {
                $access = $access - (365 * 24 * 3600); // Deux étudiants ne se sont pas connectés depuis plus d'un an.
            }

            $users[] = ['username' => sprintf('etudiant%s', ($i + 1)), 'firstname' => $firstname, 'lastname' => $lastname,
        ...
        // Attribue les droits administrateurs supplémentaires.
        $admincas = $DB->get_record('user', ['username' => 'useradmin', 'deleted' => 0], $fields = '*', MUST_EXIST);
        $adminmanual = $DB->get_record('user', ['username' => 'teacheradmin', 'deleted' => 0], $fields = '*', MUST_EXIST);
        ...
        foreach ($users as $user) {
            ...
            if ($user['username'] == 'etudiant11') {
                $wsuser = $user;
            }
        }

        // Ajoute un utilisateur webservice.
        require_once($CFG->dirroot . '/webservice/lib.php');
        $webservicemanager = new \webservice();
        $testservice = (object) ['name' => 'Service test', 'shortname' => 'servicetest', 'enabled' => 0, 'restrictedusers' => 0,
            'timecreated' => time()];
        $DB->insert_record('external_services', $testservice);
        $service = $DB->get_record('external_services', ['shortname' => 'servicetest']);
